...
class EmotionAnalyzer:
    ...
    def get_emotional_context(self, emotion_data: Dict[str, Any]) -> str:
        ...
        # Рекомендации по тону ответа (ТОЛЬКО при реальном стрессе!)
        if stress_score <= 4:
            context += "\n✅ Клиент спокоен. НЕ утешай, просто помогай.\n"
        elif stress_score >= 7:
            context += "\n⚠️ ВАЖНО: Клиент в сильном стрессе! Начни с эмоциональной поддержки, затем дай совет.\n"
            ...
            context += "\n📌 Клиент немного обеспокоен. Будь чуть мягче, но не 'терапевтируй'.\n"
